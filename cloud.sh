...
if echo "$COMPLETE_RESP" | grep -q '"status_code":[0-9][0-9][0-9]'; then
    STATUS_CODE=$(echo "$COMPLETE_RESP" | grep -o '"status_code":[0-9]*' | cut -d':' -f2)
else
    STATUS_CODE=0
fi

if [[ "$STATUS_CODE" == "200" ]] || ! echo "$COMPLETE_RESP" | grep -q '"status":"error"'; then
    echo "upload OK → 删除本地文件：$1"
    rm -f "$1"
else
    echo "upload failed → 保留本地文件：$1"
    echo "$COMPLETE_RESP"
fi